...
async function createPineconeStore(embeddings: any): Promise<VectorStore> {
  ...
  const pineconeIndex = pineconeClient.Index(config.vectorStore.pinecone!.indexName);
...
export async function clearVectorStore(): Promise<void> {
  ...
  switch (config.vectorStore.provider) {
    ...
    case "memory":
    default:
      console.warn(`Clear operation not implemented for ${config.vectorStore.provider} vector store`);
      break;
  }
}
