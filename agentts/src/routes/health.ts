    ...
    async (request, reply) => {
      try {
        ...
        try {
          ...
          if (chatService) {
            vectorStoreResponseTime =
              Number(process.hrtime.bigint() - vectorStartTime) / 1_000_000; // Convert to ms
          } else {
        ...
        try {
          ...
          if (chatService) {
            llmResponseTime =
              Number(process.hrtime.bigint() - llmStartTime) / 1_000_000; // Convert to ms
          } else {
            llmStatus = "unhealthy";
          }
        } catch (error) {
  ...
  fastify.get(
    "/health/live",
    {
      schema: {
        ...
        description:
          "Minimal health check for container orchestrators. Returns 200 if the service is running.",
        tags: ["Health"],
        response: {
          200: {
            type: "object",
  ...
  fastify.get(
    "/health/ready",
    {
      schema: {
        ...
        description:
          "Checks if the service is ready to handle requests. Validates core dependencies.",
        tags: ["Health"],
        response: {
          200: {
            type: "object",
